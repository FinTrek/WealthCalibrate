...
def get_L(n_arr, args):
    ...
    if n_arr.ndim == 2:  # This is the steady-state case
        ...
        omega_arr = np.tile(omegas.reshape((S, 1, Tpers)), (1, J, 1))
...
def get_K(b_arr, args):
    ...
    if b_arr.ndim == 2:  # This is the steady-state case
        ...
        lambda_arr = np.tile(np.reshape(lambdas, (1, J, 1)),
                             (S, 1, Tpers))
        omegas_arr = np.tile(omegas.reshape((S, 1, Tpers)),
                             (1, J, 1))
        omegas_arr2 = np.vstack((omegas_arr[1:, :, :],
                                 np.zeros((1, J, Tpers))))
        imm_rates2 = np.vstack((imm_rates[1:, :], np.zeros((1, Tpers))))
        imm_arr = np.tile(imm_rates2.reshape((S, 1, Tpers)), (1, J, 1))
        K = ((1 / (1 + g_n_arr)) *
             (((omegas_arr + omegas_arr2 * imm_arr) * lambda_arr *
               b_arr).sum(0)).sum(0))
        K_cstr = K < epsilon
        if K.min() < epsilon:
...
def get_BQ(b_arr, r_arr, args):
    ...
    if b_arr.ndim == 2:  # This is the steady-state case
        ...
        lambda_arr = np.tile(lambdas.reshape((1, J, 1)), (S, 1, Tpers))
        omegas_arr = np.tile(omegas.reshape((S, 1, Tpers)), (1, J, 1))
        BQ = ((((1 + r_arr) / (1 + g_n_arr)) *
...
def get_NX(b_arr, args):
    ...
    if b_arr.ndim == 2:  # This is the steady-state case
        ...
        lambda_arr = np.tile(np.reshape(lambdas, (1, J, 1)),
                             (S, 1, Tpers - 1))
        omegas_arr = np.tile(omegas.reshape((S, 1, Tpers - 1)),
                             (1, J, 1))
        omegas_arr2 = np.vstack((omegas_arr[1:, :, :],
                                 np.zeros((1, J, Tpers - 1))))
        imm_rates2 = np.vstack((imm_rates[1:, :],
                                np.zeros((1, Tpers - 1))))
        imm_arr = np.tile(imm_rates2.reshape((S, 1, Tpers - 1)),
                          (1, J, 1))
        NX[:-1] = -np.exp(g_y) * ((omegas_arr2 * imm_arr * lambda_arr *
                                   b_arr).sum(0)).sum(0)
        NX[-1] = NX[-2]
