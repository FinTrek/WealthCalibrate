...
def get_cnbpath(rpath, wpath, args):
    ...
    for per_rmn in range(1, S):
        ...
        for j_ind in range(J):
            ...
            if per_rmn == 1:
                ...
                b_sp1_init = np.diag(bpath[S - per_rmn + 1:, j_ind,
                                           :per_rmn - 1])
                n_s_init = \
                ...
                bn_args = (rpath[:per_rmn], wpath[:per_rmn],
                           bmat1[-per_rmn, j_ind],
                           emat[-per_rmn:, j_ind], per_rmn, beta, sigma,
                           l_tilde, chi_n_vec[-per_rmn:], b_ellip,
                           upsilon)
                results_bn = opt.root(hh.bn_errors, bn_init,
                                      args=(bn_args), tol=In_Tol)
    ...
    for t in range(T2):
        ...
        for j_ind in range(J):
            ...
            if t == 0:
                n_s_init = np.hstack((n_ss[0, j_ind],
                                      np.diag(npath[1:, j_ind,
                                                    t:t + S - 1])))
            else:
                n_s_init = np.diag(npath[:, j_ind, t - 1:t + S - 1])
            bn_init = np.hstack((b_sp1_init, n_s_init))
            bn_args = (rpath[t:t + S], wpath[t:t + S], 0.0,
                       emat[:, j_ind], S, beta, sigma, l_tilde,
                       chi_n_vec, b_ellip, upsilon)
            ...
            b_sp1_vec = np.append(bvec, 0.0)
            cvec = hh.get_cons(rpath[t:t + S], wpath[t:t + S],
                               emat[:, j_ind], b_s_vec, b_sp1_vec, nvec)
            npath[:, j_ind, t:t + S] = (DiagMaskn * nvec +
                                        npath[:, j_ind, t:t + S])
            bpath[1:, j_ind, t + 1:t + S] = \
                DiagMaskb * bvec + bpath[1:, j_ind, t + 1:t + S]
            cpath[:, j_ind, t:t + S] = (DiagMaskn * cvec +
                                        cpath[:, j_ind, t:t + S])
...
def get_TPI(bmat1, args, graphs):
    ...
    (J, E, S, T1, T2, lambdas, emat, mort_rates, imm_rates_mat,
        omega_path, g_n_path, zeta_mat, chi_n_vec, chi_b_vec, beta,
        sigma, l_tilde, b_ellip, upsilon, g_y, Z, gamma, delta, r_ss,
        BQ_ss, K_ss, L_ss, C_ss, b_ss, n_ss, maxiter, Out_Tol, In_Tol,
        xi) = args
    ...
    Y_params = (Z, gamma)
    cnb_args = (J, S, T2, lambdas, emat, omega_path, beta, sigma,
                l_tilde, b_ellip, upsilon, chi_n_vec, bmat1, n_ss,
                In_Tol)
    while (iter_TPI < maxiter) and (dist >= Out_Tol):
        ...
        MaxAbsEulErr = np.absolute(np.hstack((n_err_path,
                                              b_err_path))).max()
