...
def inner_loop(r, w, args):
    ...
    --------------------------------------------------------------------
    '''
    (c1_init, J, S, lambdas, emat, beta, sigma, l_tilde, b_ellip,
        upsilon, chi_n_vec, Z, gamma, delta, SS_tol, c1_options) = args

    rpath = r * np.ones(S)
    wpath = w * np.ones(S)
    c1_args = (np.zeros(J), emat, beta, sigma, l_tilde, b_ellip,
               upsilon, chi_n_vec, rpath, wpath, SS_tol)
    results_c1 = \
        opt.root(hh.c1_bSp1err, c1_init, args=(c1_args),
                 method='lm', tol=SS_tol, options=(c1_options))
    c1vec = results_c1.x
    cnb_args = (np.zeros(J), emat, beta, sigma, l_tilde, b_ellip,
                upsilon, chi_n_vec, SS_tol)
